"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.numerate = void 0;
function createOfLength(length, value) {
    return new Array(length).fill(value).join('');
}
function numeraliseAmount(amount, individual, half, parent) {
    // Use a basic approach for this, deal with special cases first
    if (amount === 5) {
        return half;
    }
    if (amount === 9) {
        return "" + individual + parent;
    }
    if (amount === 4) {
        return "" + individual + half;
    }
    // This is now just a case of building up the remaining strings
    return amount > 5
        ? "" + half + createOfLength(amount - 5, individual)
        : "" + createOfLength(amount, individual);
}
var decimalPlaces = [
    { individual: 'I', half: 'V', value: 1 },
    { individual: 'X', half: 'L', value: 10 },
    { individual: 'C', half: 'D', value: 100 },
    { individual: 'M', value: 1000 }, // thousands
];
function findNumeric(value, numerals) {
    // Get the current number length to allow us to figure out what decimal place we are working on
    var valueArray = value.toString().split('');
    // Get both the current decimal place and the parent, so we have the numerals we need
    var decimalPlace = decimalPlaces[valueArray.length - 1];
    var decimalParent = decimalPlaces[valueArray.length];
    // set the normalised amount for the decimal place, I.E 4 but tens would account for 40.
    var amount = Number(valueArray.splice(0, 1));
    // use our numeralise function
    numerals += numeraliseAmount(amount, decimalPlace.individual, decimalPlace.half, decimalParent === null || decimalParent === void 0 ? void 0 : decimalParent.individual);
    value = value - amount * decimalPlace.value;
    return { value: value, numerals: numerals };
}
function numerate(value) {
    if (value > 3999) {
        throw new Error('Value entered must be less than 3999');
    }
    var numerals = '';
    do {
        var result = findNumeric(value, numerals);
        value = result.value;
        numerals = result.numerals;
    } while (value > 0);
    return numerals;
}
exports.numerate = numerate;
